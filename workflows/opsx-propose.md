---
description: Propose a new change with all artifacts generated in one step based on explore.md (SSOT)
---
...
5. **Create artifacts in sequence until apply-ready**
   ...
   a. **For each artifact that is `ready` (dependencies satisfied)**:
      ...
      - Apply `context` and `rules` as constraints - but do NOT copy raw constraint blocks into the artifact.
...
7. **Generate Interactive Spec Viewer Report**
   - Compile `spec-viewer.html` for interactive review:
     ...
     ```
     *(or run `npx openspec-ex view openspec/changes/<name>`)*
