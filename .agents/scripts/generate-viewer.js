...
/**
 * OpenSpec Interactive HTML Spec Viewer Generator
 * Pure shadcn/ui Dark Aesthetic with Unified Callouts, Clean Highlights, and Pinned Comments Sidebar
 */
...
const changeId = path.basename(path.resolve(targetDir));
...
function parseMarkdown(md, sectionKey, sectionTitle) {
  ...
  let inQuote = false;
  let quoteBuffer = [];
  let listType = null; // 'tasks' | 'standard' | null
  let blockIndex = 0;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Blockquote lines (buffered together)
    if (line.startsWith('>')) {
      flushList();
      flushTable();
      inQuote = true;
...
<head>
  ...
  <style>
    ...
    * {
    ...
    .spec-block.highlight-pulse {
      animation: blockFlash 1.5s ease-in-out;
    }

    @keyframes blockFlash {
      0% { background: rgba(250, 250, 250, 0.15); box-shadow: 0 0 0 2px #fafafa; }
    ...
    .spec-block:hover .block-comment-trigger {
      opacity: 1;
    }

    .block-comment-trigger:hover {
      background: var(--secondary);
      color: var(--foreground);
      border-color: #3f3f46;
    }
    ...
    /* Unified Callouts in pure shadcn dark */
    .alert {
      display: flex;
      align-items: flex-start;
      justify-content: space-between;
...
<body>
  ...
  <!-- Main Container (Full-Width Flexbox) -->
  ...
  <script>
    ...
    function generateExportMarkdown() {
      ...
      targetComments.forEach((c, idx) => {
        ...
        if (c.refText) {
          markdown += '   - **Контекст / Текст блока**: "' + c.refText + '"\\n';
        }
      ...
      markdown += '2. Автоматически пересоберите \`spec-viewer.html\`: \`node .agents/scripts/generate-viewer.js openspec/changes/' + '${changeId}' + '\`\\n';
    ...
    // Event delegation for comment buttons on blocks
    document.addEventListener('DOMContentLoaded', () => {
      document.body.addEventListener('click', (e) => {
        ...
        if (e.target.id === 'comment-dialog') {
          closeCommentModal();
        }
        if (e.target.id === 'export-dialog') {
          closeExportModal();
        }
      });
...
const outputPath = path.join(targetDir, 'spec-viewer.html');
fs.writeFileSync(outputPath, fullHtml, 'utf8');
console.log(`[OpenSpec] Spec Viewer successfully generated: ${outputPath}`);
