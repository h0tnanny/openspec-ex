export function generateUniversalRules(): string {
  return `# OpenSpec Workflow Rules (Enhanced Edition)
...
2. **Subagent Delegation Protocol**:
   ...
   - Subagents save full briefs to \`openspec/changes/<change-id>/discovery/*.md\` and return concise ~200-word summaries.
...
4. **Interactive Spec Viewer**:
   - Compile \`spec-viewer.html\` for human-in-the-loop review.
